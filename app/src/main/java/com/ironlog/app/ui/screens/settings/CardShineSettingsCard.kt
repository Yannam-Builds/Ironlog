...
@Composable
fun CardShineSettingsCard(store: CardShineStore = CardShineRuntime.store(LocalContext.current)) {
    ...
    fun save(value: Boolean) {
        ...
        AppearancePersistence.enqueue {
            try {
                ...
                saveError = "Card shine could not be saved. Please try again."
            } finally {
                saving = false
            }
        }
    ...
    Column(Modifier.fillMaxWidth()) {
        Row(
            ...
            Column(Modifier.weight(1f), verticalArrangement = appSpacedBy(4.dp)) {
                Text("Animated card shine", color = c.text, style = MaterialTheme.typography.titleSmall)
                Text("Moving highlight on the Home workout card.", color = c.subtext, style = MaterialTheme.typography.bodySmall)
            }
